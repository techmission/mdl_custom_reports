...
define('RECORDTYPEID_COURSE_PROFESSOR', '012A0000000fqqDIAQ');
...
function run_sync() {
  /* Step 1. Query for the professors' courses in Moodle DB. */
  $sql = 'select t.userid, u.firstname, u.lastname, t.courseid, c.fullname, c.shortname, 
            if(last_login_date > 0, from_unixtime(last_login_date), 0) as last_login_date, 
            ...
            from tbl_professor_course_data t
  ...
  $soql = "select id, name, term__c, year__c, student__r.id, student__r.firstname, 
		   ...
		   where recordtypeid = '" . RECORDTYPEID_COURSE_PROFESSOR . "'";
